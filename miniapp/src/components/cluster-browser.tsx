...
type AnalyzedClusterListing = ClusterListing & { analysis_summary?: AnalysisSummary };
...
function standaloneDetail(item: ClusterFeedItem): ListingClusterDetail {
  ...
  return {
    ...
    members: [{ role: "primary", confidence: "100.00", joined_by: "exact", reasons: [], listing }],
...
function Detail({ cluster, onClose, onPatch }: { cluster: ListingClusterDetail; onClose: () => void; onPatch: (values: ClusterPatch) => void }) {
  ...
  useEffect(() => { setNote(cluster.user_state.note); }, [cluster.id, cluster.user_state.note]);
  ...
  return (
    <div className="cluster-detail-backdrop" role="presentation" onMouseDown={onClose}>
      <aside className="cluster-detail" role="dialog" aria-modal="true" aria-label="Деталі квартири та джерел" onMouseDown={(event) => { event.stopPropagation(); }}>
        ...
        <ListingAnalysisPanel listingId={listing.id} summary={analysisSummary(listing)} />
...
export function ClusterBrowser() {
  ...
  const open = useCallback(async (item: ClusterFeedItem) => {
    if (!item.listing.cluster_id) { setDetail(standaloneDetail(item)); return; }
    try { setDetail(await fetchListingCluster(item.listing.cluster_id, activeProfile?.id)); }
    catch (error) { setMessage(error instanceof ApiError ? error.message : "Не вдалося відкрити джерела квартири."); }
  }, [activeProfile?.id]);
  ...
  const update = useCallback(async (listing: ClusterListing, values: ClusterPatch) => {
    try {
      if (listing.cluster_id) {
        ...
        setItems((current) => current.map((item) => item.listing.cluster_id === updated.id ? { ...item, listing: updated.primary } : item).filter((item) => !item.listing.user_state.is_hidden));
      } else {
        ...
        setItems((current) => current.map((item) => item.listing.id === updated.id ? { ...item, listing: updated } : item).filter((item) => !item.listing.user_state.is_hidden));
        setDetail(updated.user_state.is_hidden ? null : standaloneDetail({ listing: updated, match: detail?.match ?? null }));
      }
    } catch (error) { setMessage(error instanceof ApiError ? error.message : "Не вдалося зберегти дію."); }
  }, [detail?.match]);

  return (
    <section className="cluster-browser">
      <header className="cluster-browser__hero">
        <div><span>STAGE 09 · RISK & MARKET</span><h2>Одна квартира — одна картка з аналітикою</h2><p>Дублікати об’єднано, а ринкова ціна, історія змін і пояснюваний Risk Score доступні без платних API.</p></div>
