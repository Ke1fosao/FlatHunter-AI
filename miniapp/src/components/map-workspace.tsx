...
function ListingMapSheet({
  ...
  return (
    <aside
      className="map-listing-sheet"
      role="dialog"
      aria-modal="false"
      aria-label="Квартира на карті"
    >
      <button
        type="button"
        className="map-listing-sheet__close"
        onClick={onClose}
            ...
            <div key={item.place_id}>
              ...
              <strong>
                {item.distance_km === null
                  ? "—"
                  : `${item.distance_km.toFixed(2)} км`}
              </strong>
              {item.max_distance_km !== null && (
                <small>ліміт {item.max_distance_km} км</small>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="map-listing-sheet__actions">
        <button
          type="button"
          className={listing.user_state.is_favorite ? "is-active" : ""}
          onClick={() => {
        ...
        <button
          type="button"
          className={listing.user_state.is_compared ? "is-active" : ""}
          onClick={() => {
      ...
      <Link
        ...
        className="map-listing-sheet__details"
      >
        Переглянути деталі →
      </Link>
...
export function MapWorkspace() {
  ...
  const loadProfiles = useCallback(async (signal?: AbortSignal) => {
    const result = (await fetchSearchProfiles(signal)).filter(
  ...
  const loadMap = useCallback(
    async (selectedProfile: string, signal?: AbortSignal) => {
      if (!selectedProfile) {
        setCollection({
          type: "FeatureCollection",
          features: [],
  ...
  useEffect(() => {
    if (profileId) {
      void loadMap(profileId).catch((error: unknown) => {
        setMessage(
          error instanceof ApiError
            ? error.message
            : "Не вдалося оновити карту.",
        );
      });
    }
  }, [loadMap, profileId]);
  ...
  const updateState = useCallback(
    async (action: "favorite" | "compare", value: boolean) => {
      if (!detail) {
        return;
      }
      try {
        ...
        setMessage(
          error instanceof ApiError
            ? error.message
            : "Не вдалося оновити стан квартири.",
        );
      }
    },
  ...
  return (
    <div className="map-workspace route-page map-route">
      <header className="map-workspace__toolbar">
        <div>
          <span>КАРТА ПОШУКУ</span>
          <h1>Квартири на карті</h1>
      ...
      {!activeProfile && !loading && (
        <div className="map-workspace__state">
          Створіть активний пошуковий профіль, щоб відкрити карту.
        </div>
      )}
      {message && (
        <div className="map-workspace__state is-error" role="status">
          {message}
        </div>
      )}
      ...
      <div className="map-workspace__grid">
        <section className="map-workspace__map-card">
          {loading && (
            <div className="map-workspace__loading">Завантажую геодані…</div>
          )}
          <LeafletMap
            ...
            onSelect={(id) => {
              void selectListing(id);
            }}
            onMapClick={setDraftPoint}
          />
          <div className="map-workspace__legend">
            <span>
              <i className="listing" /> Квартира
            </span>
            <span>
              <i className="favorite" /> Обране
            </span>
            <span>
              <i className="place" /> Важлива точка
            </span>
