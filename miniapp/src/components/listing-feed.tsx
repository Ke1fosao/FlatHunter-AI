...
} from "@/components/listing-analysis-panel";
...
function ListingVisual({
  ...
  return (
    <div
      className="workspace-card__visual"
      style={image ? { backgroundImage: `url(${image})` } : undefined}
    >
      {!image && <strong>{listing.rooms}к</strong>}
      {score !== undefined && (
        <span className="workspace-card__score">{score}%</span>
      )}
...
function ActionButton({
  ...
  return (
    <button
      type="button"
      className={active ? "workspace-action is-active" : "workspace-action"}
      onClick={onClick}
    >
      {label}
    </button>
...
function ListingCard({
  ...
  return (
    <article className="workspace-card">
      <button
        type="button"
        className="workspace-card__open"
        onClick={onOpen}
      ...
      <div className="workspace-card__actions">
        <ActionButton
          active={listing.user_state.is_favorite}
          label={listing.user_state.is_favorite ? "★ Обране" : "☆ В обране"}
          onClick={() => {
            onState("favorite", !listing.user_state.is_favorite);
          }}
        />
        <ActionButton
          active={listing.user_state.is_compared}
          label={
        ...
        />
        <ActionButton
          active={listing.user_state.is_hidden}
          label="Сховати"
          onClick={() => {
            onState("hide", true);
          }}
  ...
  return (
    <div className="detail-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <small>
          {listing.source_name} · {listing.city}, {listing.district}
        </small>
        <h2>{listing.title}</h2>
        <strong className="detail-panel__price">
        ...
        <ListingAnalysisPanel
          ...
          summary={listingAnalysis(listing)}
        />
        <a
          className="detail-panel__source"
  ...
  return (
    <div className="dashboard-view">
      <div className="dashboard-hero">
        <div>
          <span>FLATHUNTER CONTROL CENTER</span>
          <h2>Ваш пошук житла під контролем</h2>
          <p>
            Одна панель для найкращих збігів, обраного та швидкого порівняння.
          </p>
        </div>
        <button
          type="button"
        ...
        <div>
          {data.recent.map((item) => (
            <article key={item.id}>
              ...
              <span>{formatPrice(item.price_uah)}</span>
            </article>
          ))}
        </div>
      </div>
...
function ComparisonTable({
  ...
  onRemove: (item: ListingFeedItem) => void;
}) {
  if (items.length < 2)
    return (
      <div className="workspace-state">
        Додайте щонайменше дві квартири. Максимум — чотири.
      </div>
    );
  const rows: [string, (item: ListingFeedItem) => string][] = [
    ...
    [
      ...
      (item) =>
        item.floor
          ? `${String(item.floor)}/${item.floors_total === null ? "?" : String(item.floors_total)}`
          : "—",
    ],
...
export function ListingFeed({
      ...
      try {
        if (tab === "dashboard") {
          ...
          if (!profileId) {
            setMatches([]);
            setMessage(
              "Створіть активний пошуковий профіль, щоб побачити персональні збіги.",
            );
          } else {
            ...
            setMatches(
              response.results.filter(
                (item) => !item.listing.user_state.is_hidden,
              ),
            );
            setMessage(
              response.results.length === 0
                ? "За цими критеріями збігів немає."
                : "",
            );
          }
        } else {
          ...
          setMessage(
            response.results.length === 0
              ? tab === "favorites"
                ...
                : "Додайте квартири до порівняння."
              : "",
          );
        }
      } catch (error) {
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          setMessage(
            error instanceof ApiError
              ? error.message
              : "Не вдалося завантажити дані Mini App.",
          );
        }
      } finally {
  ...
  useEffect(() => {
    ...
    void loadProfiles(controller.signal)
      .then(() => loadCurrent(controller.signal))
      .catch(() => {
        setMessage("Відкрий Mini App у Telegram для авторизації.");
        setLoading(false);
      });
    const reload = () => {
      void loadProfiles().then(() => loadCurrent());
    };
  ...
  const currentItems = useMemo(
    () => (tab === "feed" ? matches.map((item) => item.listing) : listings),
    [listings, matches, tab],
  );

  const openDetail = useCallback(async (id: string) => {
    try {
      setDetail(await fetchListing(id));
    } catch {
      setMessage("Не вдалося відкрити деталі оголошення.");
    }
  }, []);
      ...
      try {
        ...
        setListings((current) =>
          current
            ...
            .filter(
              (item) =>
                !(tab === "favorites" && !item.user_state.is_favorite) &&
                !(tab === "comparison" && !item.user_state.is_compared) &&
                !item.user_state.is_hidden,
            ),
        );
        setMatches((current) =>
          current
            .map((item) =>
              item.listing.id === updated.id
  ...
  return (
    <section className="workspace">
      <nav className="workspace-tabs" aria-label="Розділи Mini App">
        {(["dashboard", "feed", "favorites", "comparison"] as const).map(
          (value) => (
            <button
              type="button"
              key={value}
      ...
      {tab !== "dashboard" && (
        <header className="workspace-header">
          <div>
            ...
            <p>
              Зручний мобільний простір без десятків вкладок і повторних
              пошуків.
            </p>
          </div>
          <div className="workspace-filters">
            {tab === "feed" && (
              <>
                ...
                >
                  {profiles.length === 0 && (
                    <option value="">Немає профілів</option>
                  )}
                ...
                >
                  <option value="-match_score">Найкращі</option>
                  <option value="price_uah">Дешевші</option>
                  <option value="-published_at">Новіші</option>
                </select>
              </>
            )}
            {tab === "favorites" && (
      ...
      {loading && (
        <div className="workspace-state">
          Завантаження персонального простору…
        </div>
      )}
      ...
      {!loading && tab === "feed" && matches.length > 0 && (
        <div className="workspace-grid">
          {matches.map((item) => (
            <ListingCard
              ...
              match={item.match}
              onOpen={() => {
                void openDetail(item.listing.id);
              }}
              onState={(action, value) => {
                void updateState(item.listing, action, value);
              }}
            />
          ))}
        </div>
      )}
      {!loading && tab === "favorites" && currentItems.length > 0 && (
        <div className="workspace-grid">
          {currentItems.map((item) => (
            <ListingCard
              ...
              onState={(action, value) => {
                void updateState(item, action, value);
              }}
            />
          ))}
        </div>
      ...
      {!loading && tab === "comparison" && (
        <ComparisonTable
          ...
          onRemove={(item) => {
            void updateState(item, "compare", false);
          }}
        />
      )}
      {detail && (
        <DetailPanel
          ...
          onState={(action, value) => {
            void updateState(detail, action, value);
          }}
        />
      )}
      ...
      <style jsx global>{`
        ...
        .workspace-card__visual > strong {
          font-size: 34px;
        }
        .workspace-card__visual > small {
          position: absolute;
          top: 8px;
          left: 8px;
          padding: 4px 6px;
        ...
        .detail-panel > .workspace-card__visual {
          min-height: 260px;
          margin-bottom: 18px;
        }
        .detail-panel__close {
        ...
        .comparison-table thead button:first-child {
          max-width: 180px;
          border: 0;
          background: transparent;
          color: inherit;
        ...
        @media (max-width: 520px) {
          ...
          .workspace-card__actions button:last-child {
            grid-column: 1/-1;
          }
          .dashboard-stats {
            grid-template-columns: 1fr 1fr;
