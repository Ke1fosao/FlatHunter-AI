...
export function SearchWizard({ onClose, onCreated }: Props) {
  ...
  const submit = async (event: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        reason instanceof Error ? reason.message : "Не вдалося створити пошук",
      );
    } finally {
      setBusy(false);
    }
                ...
                {step === 2 && (
                  <div className="wizard-options">
                    <button
                      type="button"
                      className={
                        form.pets.cat ? "option-chip is-active" : "option-chip"
                      }
                      onClick={() => {
                    ...
                    <button
                      type="button"
                      className={
                        form.pets.dog ? "option-chip is-active" : "option-chip"
                      }
                      onClick={() => {
                    ...
                    <button
                      type="button"
                      className={
                        form.filters.exclude_first_floor
                    ...
                    <button
                      type="button"
                      className={
                        form.filters.exclude_last_floor
                          ? "option-chip is-active"
                          : "option-chip"
                      }
                      onClick={() => {
                        patch("filters", {
                          ...form.filters,
                          exclude_last_floor: !form.filters.exclude_last_floor,
                        });
                      }}
                    >
                      Не останній поверх
                    ...
                    <button
                      type="button"
                      className={
                        form.children ? "option-chip is-active" : "option-chip"
                      }
                      onClick={() => {
                        patch("children", !form.children);
                      }}
                    >
                      Можна з дітьми
                    </button>
                  ...
                  <div className="wizard-review">
                    ...
                    <dl>
                      ...
                      <div>
                        <dt>Бюджет</dt>
                        <dd>
                          до {form.price_max?.toLocaleString("uk-UA") ?? "—"} грн
                        </dd>
                      </div>
                      <div>
                        <dt>Кімнати</dt>
                    ...
                    {Object.keys(confidence).length > 0 && (
                      <p className="confidence-note">
                        AI-поля підтверджені з рівнем упевненості. Відсутні
                        умови не були вигадані.
                      </p>
                    )}
                  </div>
                )}
