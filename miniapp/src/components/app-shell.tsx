    ...
    <main className="app-shell">
      ...
      <header className="topbar">
        ...
        <div className="header-actions">
          <div className="language-switch" aria-label={dictionary.language.label}>
          ...
          <div className="avatar" aria-label={displayName.length > 0 ? displayName : dictionary.brand.name}>
      ...
      <section className="hero-card">
        <div className="hero-content">
          <div className="eyebrow"><SparkleIcon />{dictionary.hero.eyebrow}</div>
        ...
        <div className="hero-orbit" aria-hidden="true">
          ...
          <div className="orbit-home"><HomeIcon /></div>
      ...
      <section className="stats-grid" aria-label="Statistics">
        <article><span className="stat-icon">↗</span><strong>42</strong><small>{dictionary.stats.found}</small></article>
        <article><span className="stat-icon">◎</span><strong>7</strong><small>{dictionary.stats.matches}</small></article>
        <article><span className="stat-icon">✦</span><strong>93%</strong><small>{dictionary.stats.best}</small></article>
      </section>
      ...
      <div className="content-grid">
        <section className="panel search-panel">
          ...
          <div className="search-summary">
            ...
            <div className="search-copy"><strong>{dictionary.search.name}</strong><span>{dictionary.search.criteria}</span></div>
            <span className="active-pill"><span />{dictionary.search.status}</span>
          </div>
          <div className="search-progress"><span style={{ width: "72%" }} /></div>
          <div className="search-footer"><span>{dictionary.search.newItems}</span><span>72% relevance floor</span></div>
        </section>

        <section className="panel system-panel">
          <div className="panel-heading"><div><span className="section-kicker">SYS</span><h2>{dictionary.status.title}</h2></div><span className={`system-badge system-badge--${connection}`}><StatusDot status={connection} />{statusLabel}</span></div>
          <div className="system-list">
            <div><span>{dictionary.status.backend}</span><span><StatusDot status={connection} />{statusLabel}</span></div>
            <div><span>{dictionary.status.database}</span><span><StatusDot status={health?.checks.database === "ok" ? "ready" : connection} />{health?.checks.database === "ok" ? dictionary.status.ready : statusLabel}</span></div>
            <div><span>{dictionary.status.cache}</span><span><StatusDot status={health?.checks.cache === "ok" ? "ready" : connection} />{health?.checks.cache === "ok" ? dictionary.status.ready : statusLabel}</span></div>
          </div>
          <p className={`system-message ${authFailed ? "system-message--error" : ""}`}>
            {authFailed ? dictionary.status.authError : connection === "degraded" ? dictionary.status.apiError : dictionary.status.previewDescription}
          </p>
          <span className="mode-label">{telegram.isTelegram ? dictionary.header.telegram : dictionary.header.preview}</span>
        </section>
      </div>
      ...
      <section className="listing-card">
        <div className="listing-visual">
          ...
          <div className="building building--back"><span /><span /><span /><span /><span /><span /></div>
          <div className="building building--front"><span /><span /><span /><span /><span /><span /><span /><span /></div>
          <span className="listing-badge"><SparkleIcon />{dictionary.listing.badge}</span>
      ...
      <section className="ai-summary">
        <span className="ai-mark"><SparkleIcon /></span>
        <div><span className="section-kicker">AI DAILY</span><h2>{dictionary.daily.title}</h2><p>{dictionary.daily.text}</p><small>{dictionary.daily.note}</small></div>
      </section>
